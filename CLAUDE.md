...
| `ui/paste-notice.ts` | The branded shadow-DOM panel + the persistent review badge + the "checking…" pill. All three hang from the composer's own container (`anchorContainer()` → nearest positioned, non-clipping ancestor; `anchorTo()` lays the host as a zero-height strip over the composer's top edge, card hanging off its bottom-right), so they ride with the box for free — nothing runs on the scroll path and there is no above/below choice to flip. Falls back to the viewport's bottom-right corner when no usable container exists (or the composer is itself `fixed`/`sticky`). All CSSOM styling (no `<style>`, no inline attrs → immune to page CSP) |
