...
**The user always sees who did it.** A warn or block raises a branded, shadow-DOM panel just above the composer right away, stating, in words, that the message comes from the AvoPseudo extension — *not* from the website and *not* from the AI. A pseudonymised paste instead shows a compact, persistent **review badge** in the same spot (so routine substitutions don't interrupt); clicking it opens the same full panel — which `[LABEL_xxxx]` tags were written by the extension, and exactly which values were replaced (masked, with a "show originals" toggle), plus an **Undo — paste my original** action. Closing the panel returns to the badge (not to nothing) as long as the placeholders are still in the composer, so the originals stay one click away right up until send.

The popup's **Reveal real values** covers both halves of that: `content/reveal.ts` swaps placeholders back inside finished conversation bubbles, and — since it deliberately refuses to rewrite the composer (editing the box the user is typing in would fight the editor and could send real data by accident) — an unsent paste is instead revealed by **reopening the review panel with its originals shown** (`revealPasteReview()` in `ui/paste-notice.ts`, driven from `handleReveal()` in `content/main-world.ts`). A panel opened that way does not auto-dismiss, and the popup says which of the two happened. The reveal toggle's state is `isRevealed() && (revealedCount() > 0 || isPasteReviewRevealed())` — the flag alone would claim "revealed" after a reveal that marked nothing in the page. See `ui/paste-notice.ts`.
...
| `ui/paste-notice.ts` | The branded shadow-DOM panel + the persistent review badge + the "checking…" pill. All three hang from the composer's **container** — `anchorContainer()` walks up to the nearest positioned, non-clipping ancestor and `anchorFor()` vets it (real size, enough headroom); the host is then a zero-height CSS strip across that container's top edge (`left/right/top: 0; height: 0`) with the card off its bottom-right. **Never measure the editor element**: a rich composer is an editor inside a `max-height` scroller, so once a paste overflows it the editor's own box grows past its frame and `getBoundingClientRect().top` becomes `frame.top - scrollTop` — which is what once threw the card hundreds of pixels above the composer and finally off-screen. Nothing is measured at mount, so scroll/resize/growth are all the browser's problem. Falls back to the viewport's bottom-right corner when no usable container exists (or the composer is itself `fixed`/`sticky`). All CSSOM styling (no `<style>`, no inline attrs → immune to page CSP) |
